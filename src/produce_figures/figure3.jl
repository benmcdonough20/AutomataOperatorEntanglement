...
for i in 0:2
    ...
    scatter!(emp_bins[1:end-1] .+ δ/2, dist_binned, m = :x, markersize = 5, color = seaborn[i%3+1], label = "n_A = $(i+5)")
...
savefig("figures/fig3.svg")

dist_evo_1 = []

for i in ProgressBar(6:8)
    Da = 2^i
    Db = 2^(12-i)
    X = LinearAlgebra.kron(Matrix(I, convert(Integer, Da/2), convert(Integer, Da/2)), [0.0 1.0;1.0 0.0], Matrix(I, Db, Db))
    X .*= Db / sqrt(tr(X'*X)) #normalization
...
for i in 0:2
    ...
    dist_binned = [sum(emp_bins[i] .<= dist .< emp_bins[i+1]) for i in 1:length(emp_bins)-1] / (length(dist)*δ)
    scatter!(emp_bins[1:end-1] .+ δ/2, dist_binned, m = :x, markersize = 5, color = seaborn[i%3+1], label = "n_A = $(i+6)")
    MP(x) = 1/(2π*x*β) * sqrt((λp-x)*(x-λm))
    plot!(xax, MP, linestyle = :dash, linewidth = 2, color = seaborn[i%3+1], label = "")
end
...
savefig("figures/fig3_inset.svg")