...
for (i,dist) in enumerate(dists_classical)
    ...
    hist = [sum(x[i-1] .<= spacdist .< x[i])*(nbins/xend)/length(spacdist) for i in 2:length(x)]
    scatter!(x[1:end-1] .+ xend/(2*nbins), hist, markershape = :+, color = seaborn[i], label = labels[i], markersize = 7)
end
...
for (i,dist) in enumerate(dists_evo)
    ...
    spacdist = vcat(spacings.(dist)...)
    hist = [sum(x[i-1] .<= spacdist .< x[i])*(nbins/xend)/length(spacdist) for i in 2:length(x)]
    scatter!(x[1:end-1] .+ xend/(2*nbins), hist, marker = :o, color = seaborn[i+1], label = labels[i], markerstrokewidth =0, markersize = 4)
end
